...
.PHONY: help image-build install typecheck test web sh down clean db-upgrade db-migrate db-shell
...
help:
	...
	@echo "  make install      uv sync in backend/, npm install in frontend/ and at the repo root"
	@echo ""
	@echo "Database:"
	@echo "  make db-upgrade   prisma migrate deploy"
	@echo "  make db-migrate MSG=... prisma migrate dev --create-only"
...
install:
	$(DEV) bash -c 'cd backend && uv sync --extra dev && cd ../frontend && npm install && cd /home/vscode/workspace && npm install'
...
db-upgrade:
	$(DEV) bash -c 'cd /home/vscode/workspace && PRISMA_DATABASE_URL=$$(echo "$$DATABASE_URL" | sed "s|postgresql+asyncpg://|postgresql://|") npx prisma migrate deploy'

db-migrate:
	@test -n "$(MSG)" || (echo "MSG=... is required" && exit 1)
	$(DEV) bash -c 'cd /home/vscode/workspace && PRISMA_DATABASE_URL=$$(echo "$$DATABASE_URL" | sed "s|postgresql+asyncpg://|postgresql://|") npx prisma migrate dev --name "$(MSG)" --create-only'
