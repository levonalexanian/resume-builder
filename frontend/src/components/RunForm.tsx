...
export function RunForm({
  ...
  return (
    <main className="flex h-full flex-col gap-4 overflow-y-auto p-6">
      <header>
        <h1 className="text-xl font-semibold text-slate-900">Generate a resume</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        <label className="block">
          ...
          <textarea
            ...
            required
            className="mt-1 block h-64 w-full rounded-md border border-slate-300 bg-white p-2 font-mono text-xs text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-sm font-medium text-slate-700">Company hint (optional)</span>
            <input
              type="text"
              value={companyHint}
              onChange={(e) => setCompanyHint(e.target.value)}
              className="mt-1 block w-full rounded-md border border-slate-300 bg-white p-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
              placeholder="e.g. Acme Corp"
            />
          </label>
          <label className="block">
            <span className="text-sm font-medium text-slate-700">Focus hint (optional)</span>
            <input
              type="text"
              value={focusHint}
        ...
        <div>
          <button
            type="submit"
            disabled={!canSubmit}
            className="inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
